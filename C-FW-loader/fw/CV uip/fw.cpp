...
{
	static auto hBrush = CreateSolidBrush(RGB(240, 240, 240));
	switch (message)
	{
	case WM_CTLCOLORSTATIC:
	{
		if (warningSL == reinterpret_cast<HWND>(lParam)) {
			DWORD CtrlID = GetDlgCtrlID(reinterpret_cast<HWND>(lParam));
			if (CtrlID == warningSL_)			{
		...
		if (configSL == reinterpret_cast<HWND>(lParam)) {
			DWORD CtrlID = GetDlgCtrlID(reinterpret_cast<HWND>(lParam));
			if (CtrlID == configSL_)			{
				auto hdcStatic = reinterpret_cast<HDC>(wParam);
		...
		if (deviceSL == reinterpret_cast<HWND>(lParam)) {
			DWORD CtrlID = GetDlgCtrlID(reinterpret_cast<HWND>(lParam));
			if (CtrlID == deviceSL_)			{
				auto hdcStatic = reinterpret_cast<HDC>(wParam);
				SetTextColor(hdcStatic, RGB(0, 0, 0));
...
void onLoadCMClicked(HWND hWnd)
{
	...
	SendMessage(GetDlgItem(hWnd, hProgBar_), PBM_SETPOS, 0, 0);
	SetWindowTextA(stateSB, resultLog.c_str());
	ShowWindow(stateSB, true);
}
